...
document.addEventListener('DOMContentLoaded', () => {
    ...
    auth.onAuthStateChanged(async loggedInUser => {
        if (loggedInUser) {
            ...
            await initializeAndLoadList(user);
        } else {
    ...
    // 기존 로직들을 이 함수 안에 정리합니다.
    async function initializeAndLoadList(currentUser) {
        ...
        // 함수 내부에서 이벤트 리스너를 한 번만 추가하도록 플래그 사용
        if (!listContainer.dataset.listenersAdded) {
            addEventListeners(currentUser);
            listContainer.dataset.listenersAdded = 'true';
        }
